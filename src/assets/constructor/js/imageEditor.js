 ...
 * @type {Object}
 */
import icons from "@assets/constructor/js/icons.js";
...
export default class ImageEditor {
   ...
   * @this ImageEditor
   */
  constructor(DOMContainers, imageFile, isMobile, storeFileCb, W, H) {
    W = +W;
    H = +H;
    const [cpContainer, mainContainer, toolContainer] = DOMContainers;

    // this.orderStore = ordersStore;
    this.storeFileCb = storeFileCb;
    ...
    this.imageName = imageFile?.name
      ? imageFile.name.substring(0, imageFile.name.length - 4)
      : "photo";
    ...
    this.cropper = new Cropper(
      this.initImageDOM(URL.createObjectURL(imageFile), imageFile.name),
   ...
   */
  initImageDOM(blob, name) {
    if (blob) {
      document
        .querySelector(".DocsGen__photo-changes")
        .classList.remove("hide");
      document.querySelector(".apply-done")?.classList?.remove("hide");
      name = name || "Photo";
      document.querySelector("#initial-upload span:nth-child(2)").textContent =
        name;
    }
  ...
  createCropperControls() {
    ...
    this.cropperZoomInBtn = document.getElementById("cropper-zoom-in-btn");
    this.cropperZoomOutBtn = document.getElementById("cropper-zoom-out-btn");
  ...
  createFiltersControls() {
    ...
    this.filterControlsContainer = document.querySelector(".filters-controls");
    ...
    this.filtersSliders = document.querySelectorAll(
      ...
      setTimeout(() => {
        this.cropper.getCroppedCanvas().toBlob(
          (blob) => {
            ...
            this.storeFileCb(blob);
            // store.setFileImg(blob);
            this.cropper.clear();
  ...
  createRotationControls() {
    ...
    <div class="rotation-slider-container">
      ...
      <button id="reset-rotation-btn">
        <svg width="20" height="25" viewBox="0 0 20 25" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M9.01424 10.5785C9.01425 10.6715 8.98854 10.7629 8.93978 10.8431C8.89102 10.9233 8.82099 10.9894 8.73694 11.0346C8.65288 11.0798 8.55787 11.1024 8.46175 11.1001C8.36562 11.0978 8.27188 11.0707 8.19024 11.0215L4.26646 8.6589L0.254453 6.24322C0.17662 6.19638 0.11241 6.13112 0.0678873 6.05359C0.0233649 5.97606 2.21936e-07 5.88883 2.22994e-07 5.80013C2.24051e-07 5.71143 0.0233649 5.6242 0.0678873 5.54667C0.11241 5.46914 0.17662 5.40387 0.254453 5.35704L4.26646 2.94136L8.19029 0.578766C8.27192 0.529604 8.36566 0.502462 8.46178 0.500158C8.55789 0.497852 8.6529 0.520468 8.73695 0.565657C8.821 0.610846 8.89103 0.676964 8.93979 0.757162C8.98854 0.83736 9.01425 0.928717 9.01424 1.02176L9.01424 3.52737C9.06415 3.52667 9.11318 3.52625 9.16105 3.52625C15.1472 3.52625 20 8.22251 20 14.0156C20 19.6944 15.3369 24.3192 9.51416 24.4995C9.44919 24.5024 9.38431 24.4922 9.32365 24.4695C9.263 24.4467 9.2079 24.412 9.16187 24.3676C9.11443 24.3244 9.07681 24.2722 9.0514 24.2142C9.026 24.1562 9.01338 24.0937 9.01434 24.0307L9.01434 20.802C9.01429 20.6815 9.06222 20.5656 9.14817 20.4784C9.23413 20.3912 9.35149 20.3394 9.47588 20.3338C11.1802 20.2544 12.785 19.5332 13.9472 18.3241C15.1094 17.1151 15.7371 15.5142 15.696 13.8634C15.6549 12.2126 14.9484 10.6427 13.7273 9.48926C12.5063 8.33579 10.8676 7.69014 9.16124 7.69024C9.11134 7.69024 9.06269 7.69085 9.01444 7.69155L9.01424 10.5785Z" fill="#188BF1"/>
        </svg>
      </button>
      </div>
      `;
  ...
  canvasReplace(canvas) {
    canvas.toBlob(
      ...
      this.#imageFormats[this.currentImageFormatIndex][4],
      this.#imageFormats[this.currentImageFormatIndex][4],
    );
  }
